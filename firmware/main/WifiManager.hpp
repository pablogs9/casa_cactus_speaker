...
extern const uint8_t wifiprovisioning_html_start[] asm("_binary_wifiprovisioning_html_start");
extern const uint8_t wifiprovisioning_html_end[]   asm("_binary_wifiprovisioning_html_end");
...
struct WifiManager
{
        ...
        : provisioning_ssid_(provisioning_ssid)
    {
        ...
        config_server.core_id = 1;
        ESP_ERROR_CHECK(httpd_start(&server_, &config_server));
        ...
        if (!provisioned_)
        {
            ...
            wifi_prov_scheme_softap_set_httpd_handle(&server_);
    ...
    void wait_for_connection()
    {
        ...
        if (!provisioned_)
        {
            httpd_unregister_uri_handler(server_, "/", HTTP_GET);
        }
...
private:
            ...
            httpd_req_t* req)
    {
        ...
        httpd_resp_send(req, (char*)wifiprovisioning_html_start, wifiprovisioning_html_end -
                wifiprovisioning_html_start);
