...
namespace cspot_ng
{
    struct Context
    {
        Context(const LoginBlob & blob, HTTPClient & http_client, TCPClient & tcp_client, Crypto & crypto)
            ...
            , mercury_session_(blob, shannon_tcp_client_)
        {
            crypto_.dh_init();
        }
        ...
        void spin()
        {
            if (shannon_tcp_client_.is_initialized())
            {
                ...
                mercury_session_.spin(received_data.command, received_data.data);
            }
        }
        ...
        private:
            ...
            MercurySession mercury_session_;
    };
};